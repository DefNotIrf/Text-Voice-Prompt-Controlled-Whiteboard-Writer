...
from erase_module import send_gcode_sequence
from write_module import get_gcode_for_text, send_gcode, send_gcode_command, calculate_writing_position
import threading
...
class WhiteboardGUI:
    ...
    def setup_mode_selection(self):
        ...
        ttk.Button(self.mode_frame, text="Settings", command=self.open_settings_window, width=20).pack(pady=5)
    ...
    def open_settings_window(self):
        ...
        try:
            ...
            config = {
                "center_z": 0, "write_touch_z": -4, "write_lift_z": 4,
        ...
        for i, (key, value) in enumerate(config.items()):
            ttk.Label(window, text=key.replace("_", " ").capitalize()).grid(row=i, column=0, sticky="w", padx=10, pady=5)
        ...
        ttk.Button(window, text="Save", command=save_config).grid(row=len(config)+1, column=0, columnspan=2, pady=10)

    def open_settings_window(self):
        import subprocess  # Only needed if not already imported
        from new_socket_write_com import send_gcode_command
        ...
        for i, (key, value) in enumerate(config.items()):
            ...
            def make_test_command(zval_key):
                def test():
                    try:
                        z = float(entries[zval_key].get())
                        if zval_key in ["center_z", "erase_touch_z", "erase_lift_z"]:
                            send_gcode_command(f"G90 Z{z}")
                        elif zval_key in ["write_touch_z", "write_lift_z"]:
                            send_gcode_command(f"G91 Z{z}")
                        else:
                            messagebox.showinfo("Test", f"No test command for '{zval_key}'")
                    except Exception as e:
                        messagebox.showerror("Test Failed", str(e))
                return test

            ttk.Button(window, text="Test", command=make_test_command(key)).grid(row=i, column=2, padx=5)
        ...
        ttk.Button(window, text="Save", command=save_config).grid(row=len(config)+1, column=0, columnspan=3, pady=10)

        # Extra Control Buttons
        def send_command(cmd):
            try:
                send_gcode_command(cmd)
                messagebox.showinfo("Command Sent", f"Sent: {cmd}")
            except Exception as e:
                messagebox.showerror("Command Error", str(e))

        control_frame = ttk.Frame(window)
        control_frame.grid(row=len(config)+2, column=0, columnspan=3, pady=10)

        ttk.Button(control_frame, text="Unlock", command=lambda: send_command("$x")).pack(side=tk.LEFT, padx=10)
        ttk.Button(control_frame, text="Home", command=lambda: send_command("$h")).pack(side=tk.LEFT, padx=10)
        ttk.Button(control_frame, text="Reset", command=lambda: send_command("G92 X0 Y0 Z0")).pack(side=tk.LEFT, padx=10)
    ...
    def video_loop(self):
        ...
        while True:
            ...
            if not self.calibration_complete:
                ...
                if len(image_pts) < 4:
                    if len(image_pts) == 0:
                        cv2.putText(display_frame, f"Click the bottom left corner", 
                              (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                    elif len(image_pts) == 1:
                        cv2.putText(display_frame, f"Click the bottom right corner", 
                              (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                    elif len(image_pts) == 2:
                        cv2.putText(display_frame, f"Click the top right corner", 
                              (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                    elif len(image_pts) == 3:
                        cv2.putText(display_frame, f"Click the top left corner", 
                              (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
